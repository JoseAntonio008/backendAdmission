...
const { createSchedule, fetchSchedule } = require('../services/schedule.service');
...
scheduleRouter.get('/fetch-sched',async (req,res) => {
    try {
        ...
        if (response.message != "fetch successfully") throw new Error(response.error);
        return res.status(500).json({message:"success",data:response.data})
    } catch (error) {
